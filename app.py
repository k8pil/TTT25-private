...
def start_guidance():
    ...
    if not user_resume:
        ...
        return redirect(url_for('dashboard'))
    resume_path_rel = user_resume.resume_path # Relative path from DB

    # Calculate the absolute path to the resume using app.root_path
    # app.root_path points to the directory containing app.py
    resume_path_abs = os.path.join(app.root_path, resume_path_rel)
    print(f"Absolute resume path calculated using app.root_path: {resume_path_abs}") # Debug print

    # default_resume_path = resume_path # No longer needed, using abs path
    resume_text = ""
    analysis = {}
    career_paths = [
        {
    ...
    # Initialize functions module with the ABSOLUTE resume path
    functions.initialize(resume_path_abs, resume_text,
                         analysis, career_paths, skill_keywords, session['user_id'])

    # Run the analysis and get matching career paths
    career_paths_list = functions.run()
    print("Career paths list from functions.run():")
    print(career_paths_list)

    # Now that analysis is done (inside functions.run), get the tips
    ...
    # Generate roadmaps for the recommended paths
    roadmap_dict = {}
    print("Generating roadmaps for career paths:")
    print(career_paths_list)
    for path_list_item in career_paths_list:
        # Extract just the title (e.g., "Data Scientist") from the formatted string
        # Assumes the format is "Title (Match: XX%)"
        match = re.match(r"^(.*?) \(Match: \d+%\)$", path_list_item[0])
        if match:
            career_title = match.group(1).strip()
            roadmap_dict[path_list_item[0]] = functions.generate_roadmap(career_title)
        else:
            print(f"Warning: Could not extract career title from '{path_list_item[0]}' to generate roadmap.")
