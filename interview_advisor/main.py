...
class InterviewAdvisor:
    def __init__(self, no_database=False):
        ...
        if TTS_AVAILABLE:
            ...
            print("TTS Service not available - continuing without text-to-speech capability")
        ...
        try:
            ...
            if self.speech_input.is_available():
                ...
                print("Speech input not available - continuing without speech input capability")
        except Exception as e:
        ...
        self.recommendation_engine = RecommendationEngine(self.ai_client, self.tts_service)
    ...
    def _upload_resume(self):
        ...
        try:
            ...
            extracted_text = self.resume_processor.extract_text_from_image(resume_path)
    ...
    def _start_interview(self):
        ...
        self.interview = Interview(self.ai_client, self.tts_service, self.resume_data)
        ...
        if self.db_manager:
            ...
            print(f"Interview session {self.current_session_id} created in database")
        ...
        try:
            ...
            if answer_path.endswith(('.wav', '.mp3', '.m4a')):
                if self.speech_input and self.speech_input.is_available():
                    ...
                    transcription = self.speech_input.transcribe_file(answer_path)
                    ...
                    try:
                        fluency_analysis = self.speech_input.analyze_fluency(answer_path)
                        
                        if fluency_analysis and self.db_manager and self.current_session_id:
                            self.db_manager.save_audio_metrics(self.current_session_id, fluency_analysis)
                        ...
                        print(f"Warning: Could not analyze speech fluency: {e}")
    ...
    def _end_interview(self):
        ...
        if self.db_manager and self.current_session_id:
            ...
            self.db_manager.end_session(self.current_session_id, questions_count)
            print(f"Interview session {self.current_session_id} updated in database")
        ...
        if self.db_manager and self.current_session_id:
            self.db_manager.save_analysis_results(self.current_session_id, recommendations)
            print("Interview analysis saved to database")
        
        # Print a summary of the recommendations
        summary = self.recommendation_engine.get_recommendations_summary(recommendations)
        ...
        if choice == "1":
            ...
            try:
                ...
                if duration > 0:
                    ...
                    print(f"Maximum recording duration changed to: {duration} seconds")
                else:
    ...
    def _save_posture_metrics(self, metrics):
        ...
        try:
            ...
            # Save to database
            self.db_manager.save_posture_metrics(self.current_session_id, db_metrics)
...
def parse_arguments():
    ...
    parser.add_argument("--no-database", action="store_true", help="Disable database functionality")
