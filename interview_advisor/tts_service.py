...
class TTSService:
    def __init__(self, silent_mode=False):
        ...
        self.api_key = "sk_a23a686c51fda72001c71e0e6d331ce81ef0049149a2a64b"
        ...
        if self.available_voices:
            # Try to find Roger
            roger_voices = [v for v in self.available_voices if v["name"] == "Roger"]
            ...
            if roger_voices:
                ...
                self._log(f"Voice 'Roger' not found. Default voice set to: {self.current_voice['name']}")
        else:
            self._log_error("No voices available from the API. Check your API key and internet connection.")
    ...
    def _get_voices(self) -> List[dict]:
        ...
        try:
            ...
            self._log_error("Connection error while connecting to ElevenLabs API")
            return []
    ...
    def set_voice(self, voice_name: str) -> bool:
        ...
        try:
            ...
            matching_voices = [v for v in self.available_voices if v["name"] == voice_name]
            ...
            self._log(f"Voice '{voice_name}' not found. Using '{self.available_voices[0]['name']}' instead.")
            self.current_voice = self.available_voices[0]
            return False
        except Exception as e:
    ...
    def text_to_speech(self, text: str, play_audio: bool = True) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self._log_error(f"Error generating speech: {response.status_code} - {response.text}")
                return None
    ...
    def cleanup_cache(self, max_files: int = 20) -> None:
        ...
        try:
            ...
            audio_files = [os.path.join(self.cache_dir, f) for f in files if f.endswith('.mp3')]
