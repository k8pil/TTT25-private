...
def transcribe_audio_file(audio_file, output_text_file=None):
    ...
    try:
        with sr.AudioFile(audio_file) as source:
            ...
            # Try transcribing with Google
            try:
                print("Sending to Google's API...")
...
def main():
    parser = argparse.ArgumentParser(description="Record audio and convert to text")
    parser.add_argument("--audio-output", "-a", type=str, help="Audio output file path (default: auto-generated)")
    parser.add_argument("--text-output", "-t", type=str, help="Text output file path (default: auto-generated)")
    ...
    try:
        ...
        if args.transcribe_only:
            ...
            output_file = transcribe_audio_file(args.transcribe_only, args.text_output)
        else:
            # Record audio to file
            audio_file = record_audio_to_file(args.audio_output, args.max_duration, args.device)
            
            if audio_file:
                # Transcribe audio to text
                output_file = transcribe_audio_file(audio_file, args.text_output)
            else:
                output_file = None
