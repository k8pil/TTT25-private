...
class InterviewMetricsTracker:
    ...
    def process_frame(self, frame):
        ...
        if face_results.multi_face_landmarks:
            ...
            looking_forward = self.is_facing_forward(face_results.multi_face_landmarks[0])
    ...
    def save_to_sqlite(self, is_auto_save=False):
        ...
        try:
            ...
            if is_auto_save:
                cursor.execute("DELETE FROM interview_metrics WHERE is_auto_save = 1")
    ...
    def cleanup(self):
        ...
        if self.hand_on_screen:
            self.metrics["handDetectionDuration"] += (current_time - self.hand_detection_start_time)
            
        if self.looking_away and self.looking_away_start_time:
            self.metrics["lookingAwayDuration"] += (current_time - self.looking_away_start_time)
            
        if self.bad_posture:
            self.metrics["badPostureDuration"] += (current_time - self.bad_posture_start_time)
...
def main():
    ...
    try:
        while True:
            ...
            if frame_count % (frame_interval * 10) == 0:  # Update console every ~30 frames
                print("\033[H\033[J")  # Clear console
                print(f"Hand Detection - Detection Count: {tracker.metrics['handDetectionCount']}, Total Duration: {tracker.metrics['handDetectionDuration']:.2f}s")
                print(f"Eye Contact Detection - Loss Eye Contact Count: {tracker.metrics['lossEyeContactCount']}, Looking Away: {tracker.metrics['lookingAwayDuration']:.2f}s")
                print(f"Bad Posture Monitoring - Bad Posture Count: {tracker.metrics['badPostureCount']}, Bad Posture Duration: {tracker.metrics['badPostureDuration']:.2f}s")
            ...
            if current_time - last_auto_save > auto_save_interval:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] Auto-saved metrics to database (overwrites previous auto-save)")
